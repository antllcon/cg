...
{
	m_data.center = Point2f{200.0f, 200.0f};
	m_data.radius = Point2f{50.0f, 50.0f};
	m_data.thickness = 10.0f;
	m_data.thicknessColor = Color::FromFloat(1.0f, 1.0f, 1.0f, 1.0f);
	m_data.fillColor = Color::FromRGBA(46, 204, 113, 255);
}
...
{
	if (m_data.radius.x + delta >= MIN_RADIUS)
	{
		m_data.radius.x += delta;
		m_data.radius.y += delta;
	}
